...
=================
*/
void RemoveEpair (void)
{
	epair_t	*e = mapent->epairs->next;

	if ( mapent->epairs->key )
		free( mapent->epairs->key );
	if ( mapent->epairs->value )
		free( mapent->epairs->value );
...
void ParseBrush (void)
{
	...
	assert( nummapbrushes < MAX_MAP_BRUSHES );
	...
	// read the three point plane definition
		for (i=0 ; i<3 ; i++)
		{
			if (i != 0)
				GetToken (true);
			if (strcmp (token, "(") )
				Error ("parsing brush on line %i", scriptline);
	...
	// convert to a vector / dist plane
		...
		//
		...
		}else{		// new map file	(mapversion > 220)
			GetToken (false);
			if (strcmp (token, "["))
			{
			...
			GetToken (false);
			tx.vecs[0][0] = atof(token);
			GetToken (false);
			tx.vecs[0][1] = atof(token);
			GetToken (false);
			tx.vecs[0][2] = atof(token);
			GetToken (false);
			tx.vecs[0][3] = atof(token);
			...
			GetToken (false);
			tx.vecs[1][0] = atof(token);
			GetToken (false);
			tx.vecs[1][1] = atof(token);
			GetToken (false);
			tx.vecs[1][2] = atof(token);
			GetToken (false);
			tx.vecs[1][3] = atof(token);
			...
			for (i=0 ; i<2 ; i++)
				for (j=0 ; j<3 ; j++)
					tx.vecs[i][j] = tx.vecs[i][j] / scale[i];
		}
...
qboolean	ParseEntity (void)
{
	...
	do
	{
		...
		{
			ParseEpair ();
			if(!strcmp(mapent->epairs->key, "mapversion")) {
				g_nMapFileVersion = atoi(mapent->epairs->value);
				RemoveEpair ();
			}
		}	
	} while (1);
